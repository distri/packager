(function() {
  var Packager, cacheManifest, dependencyScripts, html, jsonpWrapper, lookupCached, makeScript, omit, packageWrapper, program, reject,
    __slice = [].slice;
  ...
  lookupCached = function(cache, fullName, branch) {
    ...
    console.log(name);
    if (name) {
      return omit(cache[name], "cache");
    }
  };

  omit = function() {
    var copy, keysToOmit, object;
    object = arguments[0], keysToOmit = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
    copy = {};
    Object.keys(object).forEach(function(key) {
      if (object.hasOwnProperty(key) && !keysToOmit.include(key)) {
        return copy[key] = object[key];
      }
    });
    return copy;
  };
